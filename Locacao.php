...
class Locacao {
    ...
    //      
        if($con->execute()){
            $con2 = $pdo->prepare("SELECT COUNT(*) as totalLoc FROM locado WHERE livro = :livro");//linhas com quantidade de livros alocados
    ...
    //
        if($con2->execute()){
            $con3 = $pdo->prepare("SELECT COUNT(*) as totalRes FROM reserva WHERE livro = :livro");//linhas com quantidade de livros reservados
    ...
    function locarLivro(){
        ...
        try {
            ...
            return 'erro de conexão com o bd: ' .$erro->getMessage();
        }
        
        if($res!='pendente'){

                $con = $pdo->prepare("INSERT INTO `locado` (`livro`, `aluno`, `usuario`, `dataHoraLocacao`, `dataHoraDevolucao`, `observacao`) VALUES (:livro,:aluno,:usuario, :dataHoraLocacao,:dataHoraDevolucao,:observacao )");
                ...
                if ($con->execute()){   
                    ...
                    if($con3->rowCount() > 0){    
                        ...
                        // if ($result3 && isset($result3->reservado)) {
                            $this->removerReserva($result3->reservado);
